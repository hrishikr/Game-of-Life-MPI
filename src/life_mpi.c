...
void worker_process(int id) {
	...
	// TODO your code here
	MPI_Status status;
	int size = sizeof(params);
	MPI_Recv(&params, size, MPI_UNSIGNED_CHAR, 0, TAG_PARAMS, MPI_COMM_WORLD, &status);
	...
	size = (params.w + 2)*params.h;
	MPI_Recv(&BOARD(curr, -1, 0), size, MPI_UNSIGNED_CHAR, 0, TAG_DATA, MPI_COMM_WORLD, &status);
	...
	//// send board to main
	// TODO your code here
	MPI_Send(&BOARD(curr, -1, 0), size, MPI_UNSIGNED_CHAR, 0, TAG_RESULT, MPI_COMM_WORLD);
